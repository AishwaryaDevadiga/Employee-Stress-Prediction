...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initCharts() {
        if (chartsInitialized) {
            // If already initialized, just call resize to render properly
        ...
        fetch('/admin/analytics-data')
            ...
            .then(data => {
                ...
                const totalPredictionsVal = document.getElementById('total-predictions-val');
                const highStressVal = document.getElementById('high-stress-val');
                if (totalUsersVal) totalUsersVal.innerText = data.total_users;
                if (totalPredictionsVal) totalPredictionsVal.innerText = data.total_predictions;
                if (highStressVal) highStressVal.innerText = data.high_stress_cases;
                ...
                const stressDistCtx = document.getElementById('stressDistChart');
                if (stressDistCtx) {
                    charts.push(new Chart(stressDistCtx, {
                        type: 'doughnut',
                        data: {
                ...
                if (genderStressCtx) {
                    charts.push(new Chart(genderStressCtx, {
                        type: 'bar',
                        data: {
                            labels: ['Male', 'Female'],
                            datasets: [
                ...
                if (deptStressCtx) {
                    ...
                    charts.push(new Chart(deptStressCtx, {
                        type: 'bar',
                        data: {
                            labels: depts,
                            datasets: [
                ...
                if (monthlyTrendsCtx) {
                    ...
                    charts.push(new Chart(monthlyTrendsCtx, {
                        type: 'line',
                        data: {
                            labels: trendMonths,
                            datasets: [{
    ...
    // Initialize immediately if the analytics tab is active
    const analyticsTab = document.getElementById('analytics-tab');
    if (analyticsTab && analyticsTab.classList.contains('active')) {
        initCharts();
    }

    // Listen for tab changes and initialize/resize charts when analytics tab is shown
    if (analyticsTab) {
